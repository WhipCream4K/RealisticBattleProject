        ...
        private class SandboxAgentStatCalculateModelUpdateHorseStatsPatch
        {
            private static void Postfix(ref SandboxAgentStatCalculateModel __instance, ref Agent agent, ref AgentDrivenProperties agentDrivenProperties)
            {
                if (agent.RiderAgent != null)
                {
                    ...
                    effectiveRidingSkill = __instance.GetEffectiveSkill(
                        agent.RiderAgent.Character,
                        agent.RiderAgent.Origin,
                        agent.RiderAgent.Formation, DefaultSkills.Riding);
            ...
            private static bool PrefixDecideAgentDismountedByBlow(Agent attackerAgent, Agent victimAgent, in AttackCollisionData collisionData, WeaponComponentData attackerWeapon, ref Blow blow)
            {
                if (!blow.IsMissile)
                {
                    if (victimAgent != null && victimAgent.HasMount && victimAgent.Character != null && victimAgent.Origin != null)
                    {
                        ...
                        int ridingSkill = MissionGameModels.Current.AgentStatCalculateModel.GetEffectiveSkill(
                            victimAgent.Character,victimAgent.Origin,victimAgent.Formation, DefaultSkills.Riding);
